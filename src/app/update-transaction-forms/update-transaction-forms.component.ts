...
import { PrintedCosting } from '../v2/components/costingNew/model/printed-costing';
import { TagCosting } from '../v2/components/costingNew/model/tag-costing';
...
export class UpdateTransactionFormsComponent implements OnInit {
  ...
  draftCostingBtn: boolean = false;
  nodraftCostingBtn: boolean = false;
      ...
      if(this.everything.sampleName == 'Woven'){
        ...
        this.employeeService.getCustomerEmail(this.everything.email).subscribe(
          (data)=>{
            ...
            costingObj.companyName= data.customerName
        ...
        this.costingService.createWoven(costingObj).subscribe(
          (data)=>{
            alert('Woven Costing created')
          }
        )
      }}
      createPrintedCosting(){
      if(this.everything.sampleName=='Printed'){
        let printObj= new PrintedCosting();
        this.employeeService.getCustomerEmail(this.everything.email).subscribe(
          (data)=>{
            printObj.customerId = data.id
            printObj.customerName = data.companyName

          }
        )
        let dateArr = this.everything.date.split("-")
        printObj.entryDate = dateArr.reverse().join("-")
        printObj.tapeName = this.everything.sampName
     
        printObj.estimatedQuantity = Number(this.everything.expectedQuantity)
        printObj.status = 0
        // costingObj.executiveId = this.everything.executiveCode
        console.log(printObj)
  
        this.costingService.createPrinted(printObj).subscribe(
          (data)=>{
            alert('Printed Costing created')
          }
        )
      }}
    //   else if(this.everything.sampleName=='Tag'){
    //     let tagObj= new TagCosting();
    //     this.employeeService.getCustomerEmail(this.everything.email).subscribe(
    //       (data)=>{
    //         tagObj.customerId = data.id
    //         tagObj.customerName = data.companyName

    //       }
    //     )
    //     let dateArr = this.everything.date.split("-")
    //     tagObj.entryDate = dateArr.reverse().join("-")
    //     tagObj.status = 0
    //     // costingObj.executiveId = this.everything.executiveCode
  
    //     this.costingService.createTag(tagObj).subscribe(
    //       (data)=>{
    //         alert('Tag Costing created')
    //       }
    //     )
    //   }
    // }   
